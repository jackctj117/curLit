...
def print_label_sample(engine, n: int) -> None:  # noqa: ANN001
    ...
    if df.empty:
        ...
        return
    print(f"=== Classifier spot-check sample (n={len(df)}) — do the labels "
          "match the text? ===\n")
    for _, r in df.iterrows():
        conf = f"{r.confidence:.2f}" if pd.notna(r.confidence) else "n/a"
        print(f"[{r.classifier_version}] relevant={bool(r.is_market_relevant)} "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--sample", type=int, metavar="N", default=None,
                        help="print N random labeled posts for a "
                             "classifier-drift spot-check, then exit")
