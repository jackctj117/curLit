...
def _candles_payload(closes: list[float]) -> dict[str, Any]:
    ...
    for i, close in enumerate(closes):
        candles.append({
            ...
            "mid": {"o": str(close), "h": str(close + 1),
                    "l": str(close - 1), "c": str(close)},
            "volume": 1000 + i,
            "complete": True,
        })
...
def test_parse_empty_and_malformed():
    ...
    assert parse_daily_candles({"candles": [
...
def test_fetch_builds_request():
    ...
    out = fetch_daily_candles("NATGAS_USD", "KEY", "ACC", count=60,
                              http_get=fake_get)
    assert len(out) == 2
...
def test_refresh_fail_soft_per_instrument(prices_engine):
    ...
    counts = refresh_daily_candles(
        prices_engine, ["BROKEN", "NATGAS_USD"], "K", "A", http_get=flaky,
    )
...
def test_refresh_enables_realized_vol(prices_engine):
    ...
    vol = prov.get_realized_vol(
        "NATGAS_USD", window=20, as_of=datetime(2026, 7, 1, tzinfo=UTC),
    )
