...
from src.rates.calendar import Calendar, BusinessDayConvention, CALENDARS


class TestDayCount:
    def test_act360_one_year(self) -> None:
        assert abs(year_fraction(date(2024, 1, 1), date(2025, 1, 1), DayCountConvention.ACT_360) - 366/360) < 0.01

    def test_act365_one_year(self) -> None:
        assert abs(year_fraction(date(2024, 1, 1), date(2025, 1, 1), DayCountConvention.ACT_365) - 366/365) < 0.01

    def test_thirty360_equal_months(self) -> None:
        assert year_fraction(date(2024, 1, 1), date(2024, 2, 1), DayCountConvention.THIRTY_360) == 30/360
