...
class TestAdjustForLiquidity:
    def test_normal_window_passes_full_size(self) -> None:
        ...
        out = PositionSizer.adjust_for_liquidity(
            ...
            ts=_ts(0, 12), observed_spread_bps=1.2, profile=prof,
        )
    ...
    def test_dead_window_blocks_entry(self) -> None:
        ...
        out = PositionSizer.adjust_for_liquidity(
            base_size=10_000.0, symbol="EURUSD",
            ts=_ts(0, 22), observed_spread_bps=3.0, profile=prof,
        )
        assert out == 0.0
    ...
    def test_thin_window_halves(self) -> None:
        ...
        out = PositionSizer.adjust_for_liquidity(
            base_size=10_000.0, symbol="EURUSD",
            ts=_ts(4, 22), observed_spread_bps=1.7, profile=prof,
        )
        assert out == 5_000.0
    ...
    def test_none_profile_passthrough(self) -> None:
        ...
        out = PositionSizer.adjust_for_liquidity(
            base_size=10_000.0, symbol="EURUSD",
            ts=_ts(0, 12), observed_spread_bps=1.0, profile=None,
        )
        assert out == 10_000.0
