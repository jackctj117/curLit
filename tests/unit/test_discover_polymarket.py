...
class TestGetVolume:
    def test_volume_num_preferred(self) -> None:
...
class TestFilterAndRank:
    def test_filters_and_sorts_by_volume(self) -> None:
        markets = [
            ...
            {"slug": "no-tokens", "question": "Will Fed surprise?", "volume": 200000},
        ]
...
class TestMainEntry:
    def test_dry_run_doesnt_write(
        ...
        def fake_fetch(
            api_url: str = "", limit: int = 200, timeout_sec: float = 30
        ) -> list[dict[str, Any]]:
            return [
                {
                    "slug": "fed-test",
...
class TestExtractYesProb:
    def test_json_string(self) -> None:
        assert extract_yes_prob({"outcomePrices": json.dumps(["0.63", "0.37"])}) == pytest.approx(
            0.63
        )
...
class TestDiscoverGeoMarkets:
    def test_filters_and_tags_theme(self) -> None:
        markets = [
            {
                "slug": "hormuz-closed",
                "question": "Will the Strait of Hormuz close?",
            ...
            },
            {
                ...
                "clobTokenIds": json.dumps(["y2", "n2"]),
                "volumeNum": 500000,
            },
            {
                "slug": "low-vol",
                "question": "Taiwan blockade?",
                "clobTokenIds": json.dumps(["y3", "n3"]),
                "volumeNum": 100,
            },
    ...
    def test_limit_caps(self) -> None:
        markets = [
            {
                "slug": f"coup-{i}",
                "question": "military coup?",
                "clobTokenIds": ["y", "n"],
                "volumeNum": float(100000 - i),
            }
            for i in range(10)
        ]
...
class TestMergeGeoConfig:
    def test_appends_new_dedup(self, tmp_path: Path) -> None:
        ...
        cfg.write_text(
            yaml.safe_dump(
                {
                    "markets": [
                        {"slug": "existing", "theme": "russia_ukraine", "yes_token_id": "e"},
                    ]
                }
            )
        )
    ...
    def test_fresh_when_no_file(self, tmp_path: Path) -> None:
        ...
        new_doc, stats = merge_geo_config(
            ghost,
            [
...
class TestGeoMainEntry:
    ...
    def test_geo_dry_run_no_write(
        ...
        def fake_fetch(
            ...
            return [
                {
                    "slug": "taiwan-blockade-2026",
                    "question": "Taiwan blockade in 2026?",
        ...
        rc = main(["--mode", "geo", "--out", str(out), "--dry-run", "--min-volume", "1000"])
        assert rc == 0
