...
class PnLAttributor:
    ...
    def _persist(self, rows: list[FillAttribution]) -> None:
        ...
        with self.engine.begin() as conn:
            for r in rows:
                conn.execute(stmt, {
                    "fid": r.fill_id, "sid": r.strategy_id, "sym": r.symbol,
                    "qty": float(r.quantity), "px": float(r.fill_price),
                    "ts": r.ts, "iid": r.intent_id,
    ...
    def compute_strategy_pnl(
        ...
        for sym, qty, px, cc in rows:
            ...
            while lots and qty_remaining != 0 and (
                (qty_remaining > 0) != (lots[0][0] > 0)
            ):
        ...
        avg_open = (
            weighted_open_px / total_abs_open if total_abs_open > 0 else 0.0
        )
        return StrategyPnL(
    ...
    def list_strategies(self) -> list[str]:
        ...
        with self.engine.connect() as conn:
            rows = conn.execute(text(
                "SELECT DISTINCT strategy_id FROM strategy_fills "
    ...
    for sid in attributor.list_strategies():
        ...
        try:
            ...
            strategy_attributed_pnl.labels(
                strategy=sid, kind="realized",
            ).set(pnl.realized)
            strategy_attributed_pnl.labels(
                strategy=sid, kind="unrealized",
            ).set(pnl.unrealized)
